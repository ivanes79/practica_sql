...
create table flota.vehiculos(
	...
	constraint modelo_fk foreign key (id_modelo) references flota.modelo(id),
	constraint marca_fk foreign key (id_marca) references flota.marca(id),
	constraint color_fk foreign key (id_color) references flota.color(id),
	constraint grupo_fk foreign key (id_grupo) references flota.grupo(id)
);
...
create table flota.revisiones(
	...
	constraint matricula_fk foreign key (matricula) references flota.vehiculos(matricula),
	constraint moneda_fk foreign key (id_moneda) references flota.moneda(id)
);		
...
create table flota.seguro(
	...
	constraint matricula_fk foreign key (matricula) references flota.vehiculos(matricula),
	constraint aseguradora_fk foreign key (id_aseguradora) references flota.aseguradora(id)
);

insert into flota.modelo (nombre)
select modelo from flota.coches   
group by modelo;

insert into flota.marca  (nombre)
select marca  from flota.coches 
group by marca ;

insert into flota.grupo  (nombre)
select grupo  from flota.coches  
group by grupo ;


insert into flota.aseguradora  (nombre)
select aseguradora  from flota.coches  
group by aseguradora  ;

insert into flota.moneda (nombre)
select moneda  from flota.coches  
group by moneda  ;

insert into flota.color  (nombre)
select color  from flota.coches  
group by color  ;

insert into flota.vehiculos (matricula, fecha_compra, kms_totales, id_grupo, id_marca, id_modelo, id_color)
select matricula, fecha_compra ,kms_totales ,g.id , m2.id,m3.id ,c2.id  from flota.coches c  
inner join flota.grupo g  
on c.grupo  = g.nombre 
inner join flota.marca m2  
on c.marca  = m2.nombre 
inner join flota.modelo m3  
on c.modelo  = m3.nombre
inner join flota.color c2  
on c.color  = c2.nombre  
group by matricula, fecha_compra ,kms_totales ,g.id , m2.id,m3.id ,c2.id;


insert into flota.revisiones (fecha_revision, kms_revision, matricula, importe, id_moneda)
select  fecha_revision ,kms_revision,vehiculos.matricula,importe_revision , m.id as id_moneda  from flota.coches c  
inner join flota.moneda m 
on c.moneda = m.nombre
inner join flota.vehiculos  
on vehiculos.matricula = c.matricula 
order by fecha_revision;

insert into flota.seguro (numero_poliza, fecha_alta, matricula, id_aseguradora)
select  n_poliza ,fecha_alta_seguro , matricula ,a.id  as id_aseguradora from flota.coches c  
inner join flota.aseguradora a  
on a.nombre  = c.aseguradora
group by n_poliza ,fecha_alta_seguro , matricula ,a.id 
order by matricula; 
  


select  m.nombre as marca, m2.nombre as modelo, g.nombre as grupo, v.fecha_compra , v.matricula ,c.nombre as color ,v.kms_totales , a.nombre as aseguradora ,s.numero_poliza  from flota.vehiculos v 
inner join flota.marca m  
on m.id = v.id_marca 
inner join flota.modelo m2  
on m2.id  = v.id_modelo  
inner join flota.grupo g 
on g.id = v.id_grupo 
inner join flota.color c 
on c.id = v.id_color 
inner join flota.seguro s 
on s.matricula = v.matricula 
inner join flota.revisiones r 
on r.matricula = v.matricula 
inner join flota.aseguradora a 
on a.id = s.id_aseguradora 
inner join flota.moneda m3 
on m3.id = r.id_moneda
group by m.nombre, m2.nombre, g.nombre, v.fecha_compra , v.matricula ,c.nombre ,v.kms_totales , a.nombre ,s.numero_poliza
order by matricula 
